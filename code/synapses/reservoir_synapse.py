...
class ReservoirSynapse(nn.Module):
    ...
    def __init__(self, device='cpu', mode='rosenbaum', numberOfChemicals: int = 1, reservoir_connections = 5, non_linearity=torch.nn.functional.tanh, spectral_radius = 0.1, params: dict = {}, **options):
        """
            ...
            if 'forward' in name:
                ...
                if parameter.adapt and 'weight' in name:
                    ...
                    new_chemical = torch.einsum('i,ijk->ijk',self.y_vector, chemical) + \
                                    torch.einsum('i,ijk->ijk', self.z_vector, self.non_linearity(torch.einsum('ic,ijk->cjk', self.reservoir_matrix * self.K_matrix, chemical) + \
