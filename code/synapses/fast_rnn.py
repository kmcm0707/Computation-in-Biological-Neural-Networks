...
class FastRnn(nn.Module):
    ...
    def __init__(
        ...
        params: dict = {},
        fastRnnOptions: fastRnnOptions = None,
        ...
        :param fastRnnOptions: (fastRnnOptions) The fast RNN options. Default is None,
        """
        ...
        self.non_linearity = fastRnnOptions.nonLinear
    ...
    def init_params(self):
        ...
        self.Q_matrix = nn.Parameter(
            torch.nn.init.zeros_(
                torch.empty(
                    size=(self.numberOfChemicals, self.numberUpdateRules),
                    device=self.device,
                )
            )
        )
        ...
        self.K_matrix = nn.Parameter(
            torch.nn.init.zeros_(
                torch.empty(
                    size=(self.numberOfChemicals, self.numberOfChemicals),
                    device=self.device,
                )
            )
        )
        ...
        self.v_vector = nn.Parameter(
            torch.nn.init.ones_(torch.empty(size=(1, self.numberOfChemicals), device=self.device))
        ...
        self.tau_vector = min_tau * (base ** torch.linspace(0, 1, self.numberOfChemicals))
        ...
        if self.options.z_vector == zVectorEnum.random:
            self.z_vector = nn.Parameter(
                torch.nn.init.normal_(
                    torch.empty(size=(1, self.numberOfChemicals), device=self.device),
                    mean=0,
                    std=0.01,
                )
            )
    ...
    def fast_update(
        ...
        for name, parameter in params.items():
            if name in self.conversion_matrix:
                ...
                h_name = "slow_" + h_name
                update_vector = self.calculate_update_vector(errors, activations_and_outputs, parameter, h_name)
                ...
                if (
                    ...
                    new_chemical = torch.einsum("i,ijk->ijk", self.y_vector, chemical) + torch.einsum(
                        ...
                        self.non_linearity(
                            ...
                            + torch.einsum("ci,ijk->cjk", self.Q_matrix, update_vector)
                        ),
                    )
    ...
    def calculate_update_vector(self, error, activations_and_output, parameter, h_name) -> torch.Tensor:
        """
        ...
        if self.update_rules[13]:
            ...
            self.rflo[h_name] = (1 - 1 / self.rflo_tau) * self.rflo[h_name] + (1 / self.rflo_tau) * torch.matmul(
                diff_activation_below.T, activation_above
            )
            update_vector[i] = -error_below.squeeze(0)[:, None] * self.rflo[h_name]
            i += 1
