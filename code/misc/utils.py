...
def meta_stats(
    ...
    with torch.no_grad():
        ...
        if typeOfFeedback == typeOfFeedbackEnum.FA:
        ...
        elif typeOfFeedback == typeOfFeedbackEnum.DFA_grad_sign:
            for y_, i in zip(reversed(y), reversed(list(B))):
                e.insert(0, torch.sign(torch.matmul(e[-1], B[i]) * (1 - torch.exp(-Beta * y_))))
        ...
        elif typeOfFeedback == typeOfFeedbackEnum.scalar_sign:
            error_scalars = torch.ones((len(label), 1), device=logits.device)
            subset = torch.gather(output, 1, label.unsqueeze(1))
            mask = subset > 0.5
            mask_indices = mask.nonzero(as_tuple=True)[0]
            ...
            error_scalars[indices_more_than_half] = 0.0
            for y_, i in zip(reversed(y), reversed(list(B))):
                e.insert(0, torch.sign(error_scalars * B[i] * (1 - torch.exp(-Beta * y_))))
            ...
            for y_, i in zip(reversed(y), reversed(list(B))):
                e.insert(0, error_scalars * B[i] * (1 - torch.exp(-Beta * y_)))
        elif typeOfFeedback == typeOfFeedbackEnum.scalar_minus_one:
            error_scalars = torch.ones((len(label), 1), device=logits.device)
            subset = torch.gather(output, 1, label.unsqueeze(1))
            mask = subset > 0.5
            mask_indices = mask.nonzero(as_tuple=True)[0]
            indices_more_than_half = mask_indices.squeeze()
            error_scalars[indices_more_than_half] = -1.0
            for y_, i in zip(reversed(y), reversed(list(B))):
                e.insert(0, error_scalars * B[i] * (1 - torch.exp(-Beta * y_)))
        ...
        # -- weight update angle
        weight_angle = None
        weight_update_batch = None
        individual_CS_weight_updates = []
        if calculate_weight_update:
            ...
            for weight_update_i_array in individual_CS_weight_updates:
                temp_angle = [
                    ...
                    for Cs_weight_update_i, total_BP_weight_update_i in zip(weight_update_i_array, total_BP_weight_update)
                ]
                if individual_CS_to_batch_BP_weight_updates_angle is None:
                    individual_CS_to_batch_BP_weight_updates_angle = temp_angle
                else:
                    individual_CS_to_batch_BP_weight_updates_angle = [
                        individual_CS_to_batch_BP_weight_updates_angle_i + temp_angle_i
                        for individual_CS_to_batch_BP_weight_updates_angle_i, temp_angle_i in zip(
                            individual_CS_to_batch_BP_weight_updates_angle, temp_angle
                        )
                    ]
            individual_CS_to_batch_BP_weight_updates_angle = [
