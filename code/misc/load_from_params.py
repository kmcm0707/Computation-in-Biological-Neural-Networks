...
def load_model():
    ...
    state_dict = synapsex.state_dict()
    print(state_dict.keys())

    state_dict["P_matrix"].copy_(
        torch.nn.Parameter(
            torch.tensor(
                [
                    [
    ...
    state_dict["v_vector"].copy_(
        torch.nn.Parameter(torch.tensor([[0.18758841, 0.1872707, 0.19293375, 0.21503067, 0.20694348]]))
    )
    print(state_dict["P_matrix"])
    print(state_dict["K_matrix"])
    print(state_dict["v_vector"])
    print(state_dict.keys())
    torch.save(state_dict, "UpdateWeights.pth")
