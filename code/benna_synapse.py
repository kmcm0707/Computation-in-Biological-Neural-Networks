...
class BennaSynapse(nn.Module):
    ...
    def __init__(self, device: Literal['cpu', 'cuda'] = 'cpu', numberOfChemicals: int = 1, non_linearity=torch.nn.functional.tanh, options: dict = {}, params: dict = {}):
    ...
    def init_parameters(self, params: dict):
        ...
        self.G_vector = torch.tensor([1/min_tau *  self.tau ** (i) for i in range(self.numberOfChemicals + 1)], device=self.device)
        ...
        for name, parameter in params.items():
            if 'forward' in name:
                ...
                if parameter.adapt and 'weight' in name:
                    ...
                    inFlows = self.G_vector[0:self.numberOfChemicals - 1] * (chemical[0:self.numberOfChemicals-1] - self.C_vector[1:self.numberOfChemicals])
                    backFlows = self.G_vector[1: self.numberOfChemicals] * (chemical[1:self.numberOfChemicals] - self.C_vector[0:self.numberOfChemicals-1])
                    ...
                    newChemical[1:self.numberOfChemicals-1] = newChemical[1:self.numberOfChemicals-1] + (inFlows + backFlows) / self.C_vector[1:self.numberOfChemicals-1]
                    newChemical[self.numberOfChemicals - 1] = newChemical[self.numberOfChemicals - 1] + (outChange + inFlows[self.numberOfChemicals-1]) / self.C_vector[self.numberOfChemicals-1]
                i += 1
