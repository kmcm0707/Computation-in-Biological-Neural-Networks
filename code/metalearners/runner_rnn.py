    ...
    def __init__(
        ...
        if self.save_results:
            ...
            self.result_directory += (
                ...
                + "/"
                + str(self.options.maxTrainingDataPerClass)
            )
    ...
    def train(self):
        ...
        for eps, data in enumerate(self.metatrain_dataset):
            ...
            for time_index in range(all_logits.shape[1]):
                ...
                all_loss_meta += self.loss_func(logits, y_qry.ravel())
            all_loss_meta = all_loss_meta / all_logits.shape[1]
            ...
            if self.save_results:
                ...
                log([all_loss_meta.item()], self.result_directory + "/all_loss_meta.txt")

            line = "Train Episode: {}\tLoss: {:.6f}\tAccuracy: {:.3f}\tCurrent Training Data Per Class: {}\tAll Loss Meta: {:.6f}".format(
                eps + 1, loss_meta.item(), acc, current_training_data_per_class, all_loss_meta.item()
            )
            if self.display:
                print(line)
            ...
            if self.save_results:
                ...
                self.summary_writer.add_scalar("All Loss Meta", all_loss_meta.item(), eps)
...
def run(
    ...
    num_images: int = 0,
    continue_training_path: str = None,
) -> None:
    """
    ...
    dataset_name = "EMNIST"
    minTrainingDataPerClass = num_images
    maxTrainingDataPerClass = num_images
...
def main_runner_rnn():
    ...
    continue_training_path = os.getcwd() + "/results/rnn_fast_mode_4/0/20250924-231925"

    for i in range(len(num_images)):
        run(
            seed=0,
            display=True,
            result_subdirectory="runner_rnn_fast_mode_4_256",
